...
const DepthCarousel = ({
  ...
  const data = useMemo(
    () => (Array.isArray(items) && items.length ? items : []).map(normalizeItem),
    [items]
  );
  ...
  cfgRef.current = {
    count, depth, spread, tilt, tiltDirection,
    visibleCards, falloff, blur, duration, ease,
    loop, cardWidth, autoplayDelay,
  };
  ...
  useEffect(() => {
    ...
    const start = () => {
      stop();
      autoTimerRef.current = setInterval(() => { if (!hovered && !focused) navigateBy(1); }, Math.max(cfgRef.current.autoplayDelay, 600));
    };
    ...
    >
      ...
      {showControls && count > 1 && (
        <>
          <button
            type="button"
            className="absolute left-4 top-1/2 z-[3000] grid h-[42px] w-[42px] -translate-y-1/2 place-items-center rounded-full border border-[rgba(212,163,89,0.3)] bg-[rgba(18,20,26,0.7)] text-[#F3C87A] backdrop-blur-md transition-all duration-200 hover:border-[#F3C87A] hover:bg-[rgba(58,39,13,0.8)] hover:scale-110 active:scale-95"
          ...
          <button
            type="button"
            className="absolute right-4 top-1/2 z-[3000] grid h-[42px] w-[42px] -translate-y-1/2 place-items-center rounded-full border border-[rgba(212,163,89,0.3)] bg-[rgba(18,20,26,0.7)] text-[#F3C87A] backdrop-blur-md transition-all duration-200 hover:border-[#F3C87A] hover:bg-[rgba(58,39,13,0.8)] hover:scale-110 active:scale-95"
      ...
      {showIndicators && count > 1 && (
        <div
          className="absolute bottom-4 left-1/2 z-[3000] flex -translate-x-1/2 gap-2 rounded-full bg-[rgba(14,16,22,0.5)] px-3 py-2 backdrop-blur-sm"
          role="tablist"
          aria-label="Slides"
        >
          {data.map((_, i) => (
            <button
              ...
              className={`h-[7px] cursor-pointer rounded-full transition-[width,background] duration-[250ms] ${
                active === i ? "w-5 bg-[#F3C87A]" : "w-[7px] bg-white/30"
              }`}
